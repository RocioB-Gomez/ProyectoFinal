...
const Alumno = {
    ...
    crearAlumno: async (dni, anio_ingreso, nombre, apellido, curso, fk_tutor) => {
        const query = 'INSERT INTO alumno (dni, anio_ingreso, curso, fk_tutor, nombre, apellido) VALUES (?, ?, ?, ? ,?,?)';
        const insertado = 'SELECT * FROM alumno WHERE id_alumno = LAST_INSERT_ID()';
        try {
            await db.execute(query, [dni, anio_ingreso, curso, fk_tutor, nombre, apellido]);
